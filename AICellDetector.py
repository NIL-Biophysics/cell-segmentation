...
def open_image():
    ...
    if file_path:
        ...
        text_widget.insert(tk.END, coords_text)
        text_widget.config(state=tk.DISABLED)

        mask = Image.fromarray(mask)
        mask.save("y_pred.png")
        mask = mask.resize((width, height), Image.LANCZOS)
        mask_tk = ImageTk.PhotoImage(mask)
        label_processed_image.config(image=mask_tk)
        label_processed_image.image = mask_tk

    end_time = time.time()
    print(f"Время выполнения: {end_time - start_time} секунд.")

root = tk.Tk()
root.title("AI Cell Detector")
...
button_open = tk.Button(image_frame, text="Open Image", command=open_image)
button_open.pack()

label_image = tk.Label(image_frame, text="Source Image")
label_image.pack()

label_processed_image = tk.Label(image_frame, text="Post-Processed Image")
label_processed_image.pack()

text_widget = tk.Text(frame, width=40, height=10, state=tk.DISABLED)
